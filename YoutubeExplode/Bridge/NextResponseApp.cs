...
internal partial class NextResponseApp(JsonElement content)
{
    ...
    private JsonElement? ContentRoot =>
        content
            ...
            ?.GetPropertyOrNull("endScreen")
            ?.GetPropertyOrNull("watchNextEndScreenRenderer");

    [Lazy]
    public IReadOnlyList<VideoData> Videos =>
        ContentRoot
            ?.EnumerateDescendantProperties("endScreenVideoRenderer")
...
internal partial class NextResponseApp
{
...
internal partial class NextResponseApp
{
    public static NextResponseApp Parse(string raw) => new(Json.Parse(raw));
}
